...
use jmap_proto::{
    ...
    object::calendar_event::{CalendarEvent, CalendarEventComparator, CalendarEventFilter},
...
pub trait CalendarEventQuery: Sync + Send {
    fn calendar_event_query(
        &self,
        request: QueryRequest<CalendarEvent>,
...
impl CalendarEventQuery for Server {
    async fn calendar_event_query(
        &self,
        mut request: QueryRequest<CalendarEvent>,
        access_token: &AccessToken,
        ...
        for cond in std::mem::take(&mut request.filter) {
            match cond {
                Filter::Property(cond) => match cond {
                    ...
                    CalendarEventFilter::Email(email) => filters.push(query::Filter::eq(
                        ContactField::Email,
                        sanitize_email(&email).unwrap_or(email).into_bytes(),
                    )),
                    CalendarEventFilter::Text(value) => filters.push(query::Filter::has_text(
                        ContactField::Text,
                        value.to_lowercase(),
                    )),
                    CalendarEventFilter::CreatedBefore(before) => filters.push(query::Filter::lt(
                        ContactField::Created,
                        (before.timestamp() as u64).serialize(),
                    )),
                    CalendarEventFilter::CreatedAfter(after) => filters.push(query::Filter::gt(
                        ContactField::Created,
                        (after.timestamp() as u64).serialize(),
                    )),
                    CalendarEventFilter::UpdatedBefore(before) => filters.push(query::Filter::lt(
                        ContactField::Updated,
                        (before.timestamp() as u64).serialize(),
                    )),
                    CalendarEventFilter::UpdatedAfter(after) => filters.push(query::Filter::gt(
                        ContactField::Updated,
                        (after.timestamp() as u64).serialize(),
                    )),
                    unsupported => {
        ...
        if let Some(paginate) = paginate {
            ...
            for comparator in request
                .sort
                .filter(|s| !s.is_empty())
                .unwrap_or_else(|| vec![Comparator::descending(CalendarEventComparator::Updated)])
            {
